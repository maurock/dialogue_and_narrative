...
def sharedtask1_metrics(prediction_json, split, cache_dir=None):
    ...
    #     metric.add_batch(predictions=[predictions[idx]], references=[references[idx]])
    #     final_score = metric.compute()
    #     f1_list.append(final_score['f1'])
    # highest_f1_pos = np.argmax(f1_list)
    # highest_f1 = np.amax(f1_list)
...
def main():
    ...
    if args.task == "subtask1":
        result = sharedtask1_metrics(args.prediction_json, args.split, args.cache_dir)
    else:
